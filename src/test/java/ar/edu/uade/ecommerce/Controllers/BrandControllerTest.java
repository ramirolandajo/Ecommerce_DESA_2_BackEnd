...
class BrandControllerTest {
    ...
    void testUpdateBrandFromMock() {
        BrandDTO mockBrand = new BrandDTO(5L, "Motorola", null);
        KafkaMockService.BrandSyncMessage message = new KafkaMockService.BrandSyncMessage("BrandSync", new KafkaMockService.BrandSyncPayload(List.of(mockBrand)), "2025-09-02T08:29:02.072020100");
        when(kafkaMockService.getBrandsMock()).thenReturn(message);
        Brand brand = new Brand();
        brand.setId(5);
    ...
    void testSyncBrandsFromMock_EmptyExistingBrands_shouldAddBrand() {
        ...
        KafkaMockService.BrandSyncMessage message = new KafkaMockService.BrandSyncMessage("BrandSync", new KafkaMockService.BrandSyncPayload(List.of(mockDto)), "2025-09-02T08:29:02.072020100");
        when(kafkaMockService.getBrandsMock()).thenReturn(message);
        // Primera llamada devuelve lista vacía, segunda llamada devuelve la lista con la marca guardada
        Brand brand = new Brand();
        brand.setId(8);
        brand.setName("Alcatel");
        brand.setActive(true);
        when(brandService.getAllBrands()).thenReturn(List.of(), List.of(brand));
        when(brandService.saveBrand(any(Brand.class))).thenReturn(brand);
        List<BrandDTO> result = brandController.syncBrandsFromMock();
